...
def UAMT20LDataset(dataset_root):
    name_list=os.listdir(os.path.join(dataset_root, 'data_seq'))
    name_list.sort()
    a=len(name_list)
    b=[]
    for i in range(a):
...
class UAMT20L(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(UAMT20L, self).__init__(name, dataset_root)

        meta_data = UAMT20LDataset(dataset_root)
